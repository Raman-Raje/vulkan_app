...
//     // image size
//     const int height = 5, width = 5, channel = 4, length = 100;
...
//     VulkanImage outputTexture(device, physicalDevice, width, height, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);    

//     // Fill buffer with data
//     std::vector<float> inputData(bufferSize / sizeof(float), 1.0f); // Fill with 1.0f

//     // Map the data to buffer
//     void* mappedData;
//     vkMapMemory(device, inputBuffer.getBufferMemory(), 0, bufferSize, 0, &mappedData);
//     std::memcpy(mappedData, inputData.data(), static_cast<size_t>(bufferSize));
//     vkUnmapMemory(device, inputBuffer.getBufferMemory());

//     // Pipeline creation
//     VulkanPipeline vulkanPipeline(device);

//     // Define Descriptor Set Layout Bindings for texture and buffer
//     std::vector<VkDescriptorSetLayoutBinding> bindings = {
//         { 0, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1, VK_SHADER_STAGE_COMPUTE_BIT, nullptr },   // texture
//         { 1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1, VK_SHADER_STAGE_COMPUTE_BIT, nullptr }  // buffer
//     };
//     vulkanPipeline.createDescriptorSetLayout(bindings); 

//     // Create pipeline with the new shader
//     vulkanPipeline.createPipeline("/local/mnt/workspace/ramashin/tests/vulkan/testSpirv/spirv/tvmgen_default_fused_layout_transform_kernel0_spv.spv", "tvmgen_default_fused_layout_transform_kernel0");

//     // Allocate Descriptor Set and Update with Resources
//     std::vector<VkDescriptorPoolSize> poolSizes = {
//         { VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1 },  // Texture
...
//     // Transition the output texture to GENERAL layout for read/write access in the shader
//     VulkanUtils::transitionImageLayout(device, vulkanDevice.getCommandPool(), vulkanDevice.getComputeQueue(),
//                                        outputTexture.getImage(), VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL);
                                       
//     // Dispatch the compute shader
//     commandBuffer.dispatchCompute(vulkanPipeline.getPipeline(), vulkanPipeline.getPipelineLayout(), descriptorSet, 4, 1, 1);  // Adjusted for workgroup size (25,1,1)
...
//     // Transition the output texture layout to TRANSFER_SRC_OPTIMAL for copying to buffer
//     VulkanUtils::transitionImageLayout(device, vulkanDevice.getCommandPool(), vulkanDevice.getComputeQueue(),
//                                        outputTexture.getImage(), VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);
...
//                                    outputTexture.getImage(), stagingBuffer.getBuffer(), width, height);
...
//         float expectedValue = 1.0f; // Assuming your shader logic transforms data to 1.0f
...
int main() {
    ...
    // The shader operates on 25 workgroup threads, each processing 4 floats from the buffer.
    // The buffer needs at least 100 elements (4 floats for each of the 25 threads).
    const int length = 100;  // 100 float elements
    VkDeviceSize bufferSize = length * sizeof(float);

    // Image is a 5x5 texture (as indicated by the shader logic)
    const int height = 5, width = 5, channel = 4;  // 5x5 texture with 4 channels (vec4)
    VkDeviceSize imageSize = height * width * channel * sizeof(float);

    // create input buffer
    VulkanBuffer inputBuffer(device, physicalDevice, bufferSize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, 
                             VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
    ...
    for (int i = 0; i < length; ++i) {
        inputData[i] = static_cast<float>(i);  // Fill with values 0, 1, 2, ..., 99
    }
    ...
    // Define Descriptor Set Layout Bindings for the buffer and image
    std::vector<VkDescriptorSetLayoutBinding> bindings = {
        { 0, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1, VK_SHADER_STAGE_COMPUTE_BIT, nullptr },   // Image
        { 1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1, VK_SHADER_STAGE_COMPUTE_BIT, nullptr },  // Buffer
    };
    vulkanPipeline.createDescriptorSetLayout(bindings);

    // Create pipeline with the buffer-to-image shader
    vulkanPipeline.createPipeline("/local/mnt/workspace/ramashin/tests/vulkan/testSpirv/spirv/tvmgen_default_fused_layout_transform_kernel0_spv.spv", "tvmgen_default_fused_layout_transform_kernel0");
    ...
    // Dispatch the compute shader
    // We use 1 workgroup since the shader has 25 invocations (local_size_x = 25)
    commandBuffer.dispatchCompute(vulkanPipeline.getPipeline(), vulkanPipeline.getPipelineLayout(), descriptorSet, 1, 1, 1);
    ...
    float* outputData = static_cast<float*>(mappedData);
    for (int i = 0; i < width * height * channel; i += 4) {
        // Compare output data based on expected results
        std::cout << "Pixel " << i / 4 << ": (" << outputData[i] << ", " << outputData[i + 1] << ", " << outputData[i + 2] << ", " << outputData[i + 3] << ")" << std::endl;
    }
